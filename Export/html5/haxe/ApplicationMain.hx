...
@:dox(hide) class ApplicationMain {
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("client.Main"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess client.Main.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"client.Main\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"client.Main\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends client.Main {}
